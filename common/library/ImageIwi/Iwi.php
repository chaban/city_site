<?php namespace Iwi;
use Vendors\Image\Image, Models\Storage, Vendors\Image\Image_Driver;
class Iwi extends Image
{
  ...
  {
    $folder[] = __dir__.'/../../../public/uploads/site/cache';
    $folder[] = substr($this->hash(), 0, 2);
    $folder[] = substr($this->hash(), 2, 2);

    $path[] = "uploads/site/cache";
    //$path[] = __dir__.'../../../images/site/cache';
    $path[] = substr($this->hash(), 0, 2);
    $path[] = substr($this->hash(), 2, 2);

    $path = implode("/", $path);
    $folder = implode("/", $folder);
    //echo "folder ",var_dump($folder);
  ...
  public function createOrNone()
  {
    //$this->verifyTable();
    $hash = $this->hash();
    $result = \Models\Storage::findFirst(array(
        "conditions" => "key = ?1",
        "bind"       => array(1 => "$hash"),
  ...
  public function __construct($image, $config = null)
  {
    ...
    $this->driver = new \Vendors\Image\Drivers\Image_GD_Driver($this->config['params']);
