...
class SlugBehavior extends \Phalcon\Mvc\Model\Behavior
{
	/**
  ...
  /**
	...
	{
		//$model = $this->getOwner();
		if ($this->getIsNewRecord($model) || $this->overwrite === true)
		{
			$slug = $this->getUniqueSlug($model);
			$model->{$this->slug_col} = $slug;
		}
	}
	 ...
	 * @param string $slug
	 */
	public function getUniqueSlug($model)
	{
		//$model = $this->getOwner();
		$slug = $this->getSlug($model->{$this->title_col});
		//$this->pk_col = $model->getMetaData()->tableSchema->primaryKey;
		//$metaData = $model->getDI()->getModelsMetadata();
		//$this->pk_col = $metaData->getIdentityField($model);
		$model->{$this->slug_col} = $slug;

		$matches = $this->getMatches($model);
	 ...
	 * @param string $title
	 */
	public function getMatches($model)
	{
		//$owner = $this->getOwner();
		/*$slugs = $model::find(array('select' => $this->slug_col . ' , ' . $this->title_col . ' , ' . $this->pk_col,
				'conditions' => $this->slug_col . " LIKE '%" . $slug . "%'"));*/
		$slugs = $model::query()
    ->where($this->slug_col . " LIKE '%" . ':slug:' . "%'")
    ->bind(array("slug" => $model->slug))
    ->execute();
		return $slugs;
	}
	...
	public function getIsNewRecord($model)
	{
		return $model->getDirtyState() == \Phalcon\Mvc\Model::DIRTY_STATE_TRANSIENT;
	}
