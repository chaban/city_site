...
class CFileHelper
{
    ...
    while (($file = readdir($folder)) !== false)
    {
      ...
      if (self::validatePath($base, $file, $isFile, $fileTypes, $exclude))
      {
        if ($isFile)
        {
          ...
          if ($level)
            self::copyDirectoryRecursive($path, $dst . DIRECTORY_SEPARATOR . $file, $base . '/' . $file, $fileTypes, $exclude,
              $level - 1, $options);
      }
    }
    closedir($folder);
  ...
  /**
   * Deletes directory and files in it recursively
  ...
  /**
   * Определяет наличие кириллических символов в тексте
   * @param string $text текст
   **/
  public static function is_kir($text = null)
  {
    $maska = "/([ієїа-я])/i";
    if ($text !== null)
    {
      preg_match($maska, $text, $match);
      if (!empty($match[0]))
        return true;
      else
        return false;
