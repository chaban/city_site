...
class Time
{
	 ...
	 */
	public static function nice($time)
	{
	...
	public static function niceShort($dateString = null)
	{
		...
		{
			$ret = sprintf('Сегодня, %s', date("H:m", $date));
		} elseif (self::wasYesterday($date))
		{
			$ret = sprintf('Вчера, %s', date("H:m", $date));
		} else
		{
			$ret = date("d M y H:m", $date);
		}
	...
	function timeAgoInWords($dateTime, $options = array())
	{
		...
		if ($diff >= 604800)
		{
			...
			list($future['H'], $future['i'], $future['s'], $future['d'], $future['m'], $future['Y']) = explode('/', date('H/i/s/d/m/Y',
				$futureTime));
		...
		} else
		{
			...
			} elseif (abs($hours) > 0)
			{
				...
				$relativeDate .= $minutes > 0 ? ($relativeDate ? ', ' : '') . $minutes . ' ' . ($minutes == 1 ? 'minute' :
					'minutes') : '';
