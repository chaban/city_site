<?php namespace Vendors\Image;
use Vendors\Image\Image_Driver;
/**
...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ($check === FALSE)
			throw new \Phalcon\Mvc\Model\Exception('image getimagesize missing');

		// Check to make sure the image exists
		if ( ! is_file($image))
			throw new \Phalcon\Mvc\Model\Exception('image file not found');
		...
		if ( ! is_array($image_info) OR count($image_info) < 3)
			throw new \Phalcon\Mvc\Model\Exception('image file unreadable');

		// Check to make sure the image type is allowed
		if ( ! isset(Image::$allowed_types[$image_info[2]]))
			throw new \Phalcon\Mvc\Model\Exception('image type not allowed');
		...
		//$driver = 'Shop\Vendors\Image\Drivers\Image_'.ucfirst($this->config['driver']).'_Driver';

        // Load the driver
        //Yii::import("application.extensions.iwi.vendors.image.drivers.$driver");

		// Initialize the driver
		$this->driver = \Shop\Vendors\Image\Drivers\Image_GD_Driver($this->config['params']);

		// Validate the driver
		if ( ! ($this->driver instanceof Image_Driver))
			throw new \Phalcon\Mvc\Model\Exception('image driver must be implement Image_Driver class');
	}
	...
	public function __get($property)
	{
		...
		{
			throw new \Phalcon\Mvc\Model\Exception('invalid property');
		}
	}
	...
	public function resize($width, $height, $master = NULL)
	{
		if ( ! $this->valid_size('width', $width))
			throw new \Phalcon\Mvc\Model\Exception('image invalid width');

		if ( ! $this->valid_size('height', $height))
			throw new \Phalcon\Mvc\Model\Exception('image invalid height');

		if (empty($width) AND empty($height))
			throw new \Phalcon\Mvc\Model\Exception('image invalid dimensions');
		...
		elseif ( ! $this->valid_size('master', $master))
			throw new \Phalcon\Mvc\Model\Exception('image invalid master');
	...
	{
		if ( ! $this->valid_size('width', $width))
			throw new \Phalcon\Mvc\Model\Exception('image invalid width', $width);

		if ( ! $this->valid_size('height', $height))
			throw new \Phalcon\Mvc\Model\Exception('image invalid height', $height);

		if ( ! $this->valid_size('top', $top))
			throw new \Phalcon\Mvc\Model\Exception('image invalid top', $top);

		if ( ! $this->valid_size('left', $left))
			throw new \Phalcon\Mvc\Model\Exception('image invalid left', $left);
	...
	public function flip($direction)
	{
		if ($direction !== self::HORIZONTAL AND $direction !== self::VERTICAL)
			throw new \Phalcon\Mvc\Model\Exception('image invalid flip');
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		if ( ! is_writable($dir))
			throw new \Phalcon\Mvc\Model\Exception('image directory unwritable');
